...
function ReviewPageInner() {
  ...
  // Auto-open fields when arriving from the processing page (fileId already in URL)
  useEffect(() => {
    if (fileId) {
      setStatus("done");
    }
  ...
  // Manual entry mode — no AI. Opens the fields form immediately.
  // To re-enable AI extraction later, restore the async fetch to /api/extract here.
  function onExtract() {
  ...
  async function createCustomer() {
    ...
    const res  = await fetch("/api/customers", {
      ...
      body: JSON.stringify({ fullName: newCustName, phone: newCustPhone, companyName: newCustCompany }),
    });
    const data = await res.json();
  ...
  return (
    <div style={s.page}>
      ...
      <header style={s.header}>
        ...
        {/* Only the back-to-processing button */}
        <a href="/processing" style={s.backBtn}>← Processing</a>
      </header>
      ...
      {!fileId && (
        <div style={s.uploadBar}>
          ...
          {hasDoc && (
            ...
            >
              {status === "loading" ? "Opening…" : "Fill in Fields"}
            </button>
          )}
        </div>
      )}
        ...
        <div style={s.columns}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            ...
            <div style={s.card}>
              ...
              {status === "idle"   && <div style={s.hintBox}>Click <strong>Fill in Fields</strong> to open the entry form.</div>}
            ...
            <div style={s.card}>
              ...
              <p style={s.hintSmall}>Link this document to a customer so it appears in their profile.</p>
              {selectedCust ? (
                <div style={s.selBox}>
                  <div style={{ flex: 1 }}>
                ...
                <>
                  <input style={s.searchInput} placeholder="Search by name, company, phone…" value={custSearch} onChange={(e) => setCustSearch(e.target.value)} />
...
const s: Record<string, React.CSSProperties> = {
  ...
  backBtn:     { padding: "10px 16px", borderRadius: 10, background: "white", color: "#475569", border: "1px solid rgba(15,23,42,0.12)", fontWeight: 600, fontSize: 13, textDecoration: "none" },
