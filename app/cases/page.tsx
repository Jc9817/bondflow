...
export default function CasesPage() {
  ...
  const filtered = cases.filter((c) =>
    (filterStatus === "ALL" || c.caseStatus === filterStatus) &&
  ...
  return (
    <div style={s.page}>
      ...
      <div style={s.body}>
        <div style={s.topbar}>
          <div>
            <div style={s.pageTitle}>Cases</div>
            <div style={s.pageSub}>Manage and track all cases</div>
          </div>
        ...
        <div style={s.filterRow}>
          {(["ALL", "OPEN", "CLOSED"] as const).map((f) => (
            <button key={f} style={{ ...s.filterBtn, ...(filterStatus === f ? s.filterBtnActive : {}) }} onClick={() => setFilterStatus(f)}>
              {f}{f !== "ALL" && <span style={{ marginLeft: 6, fontSize: 11 }}>{cases.filter((c) => c.caseStatus === f).length}</span>}
            </button>
          ))}
          <div style={s.filterDivider} />
          {/* Case type filter */}
          {["ALL", ...CASE_TYPES].map((t) => (
            <button
              key={t}
              style={{ ...s.filterBtn, ...(filterType === t ? s.filterTypeActive : {}) }}
              onClick={() => setFilterType(t)}
            >
              {t === "ALL" ? "All Types" : t}
              {t !== "ALL" && (
                <span style={{ marginLeft: 6, fontSize: 11 }}>
                  {cases.filter((c) => c.caseType === t).length}
                </span>
              )}
            </button>
          ))}
          <span style={{ marginLeft: "auto", fontSize: 13, color: "#94a3b8" }}>{filtered.length} case{filtered.length !== 1 ? "s" : ""}</span>
        </div>
              ...
              <div key={c.id} style={s.caseCard}>
                ...
                <div style={s.caseRight}>
                  <div style={{ textAlign: "center" as const }}>
                    <span style={{ display: "block", fontWeight: 800, fontSize: 18, color: "#0f172a", lineHeight: 1 }}>{c.jobs.length}</span>
                    <span style={{ display: "block", fontSize: 11, color: "#94a3b8", marginTop: 2 }}>doc{c.jobs.length !== 1 ? "s" : ""}</span>
                  </div>
                  <div style={{ fontSize: 12, color: "#94a3b8" }}>{new Date(c.createdAt).toLocaleDateString()}</div>
...
const s: Record<string, React.CSSProperties> = {
  ...
  body:           { flex: 1, padding: "24px 28px", display: "flex", flexDirection: "column", gap: 16 },
  ...
  filterBtn:      { padding: "7px 14px", borderRadius: 20, background: "white", color: "#64748b", border: "1px solid rgba(15,23,42,0.12)", fontWeight: 600, fontSize: 12, cursor: "pointer", whiteSpace: "nowrap" as const },
  filterBtnActive:{ background: "#0f172a", color: "white", border: "1px solid #0f172a" },
  filterTypeActive:{ background: "#2563eb", color: "white", border: "1px solid #2563eb" },
  filterDivider:  { width: 1, height: 20, background: "rgba(15,23,42,0.15)", margin: "0 4px", flexShrink: 0 },
